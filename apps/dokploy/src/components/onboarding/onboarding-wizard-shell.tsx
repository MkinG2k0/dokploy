		...
		<div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 md:flex-row md:gap-10">
			<aside className="w-full shrink-0 md:w-56">
				...
				<nav className="flex flex-col gap-1" aria-label="Onboarding steps">
					{STEP_IDS.map((id) => {
						const done = id < currentStep || (id === 3 && step3Skipped)
						const active = id === currentStep
						const skippedLabel = id === 3 && step3Skipped
						const clickable = id <= maxReachableStep && !skippedLabel
						return (
							<button
								key={id}
								type="button"
							...
							>
								...
								<span className="truncate">
									...
									{id === 2 && t('stepServer')}
									{id === 3 &&
										(step3Skipped ? `— ${t('skip')}` : t('stepSettings'))}
									{id === 4 && t('stepDeploy')}
								</span>
							</button>
						)
