...
const assertServerNotPlanBlocked = (srv: { planAccessBlocked: boolean }) => {
	if (IS_CLOUD && srv.planAccessBlocked) {
		throw new TRPCError({
			code: "FORBIDDEN",
			message:
				"This server is locked on your current plan. Upgrade billing or delete excess servers.",
		});
	}
};
...
export const serverRouter = createTRPCRouter({
	create: withPermission("server", "create")
		...
		.mutation(async ({ ctx, input }) => {
			try {
				...
				if (IS_CLOUD) {
					await syncPlanAccessBlockedForOwner(ctx.user.ownerId);
				}
				return project;
			} catch (error) {
				throw new TRPCError({
	...
	getCloudServerLimit: withPermission("server", "read").query(
		async ({ ctx }) => {
			if (!IS_CLOUD) {
				return { allowed: null as number | null };
			}
			const admin = await findUserById(ctx.user.ownerId);
			return { allowed: admin.serversQuantity };
		},
	),
	...
	setup: withPermission("server", "create")
		...
		.mutation(async ({ input, ctx }) => {
			try {
				...
				assertServerNotPlanBlocked(server);
