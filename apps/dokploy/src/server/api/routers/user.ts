...
type UserGetMemberRow = InferResultType<
  "member",
  { user: { with: { apiKeys: true } } }
>;

type UserGetOutput =
  | undefined
  | (UserGetMemberRow & { isSuperAdmin?: boolean });
...
export const userRouter = createTRPCRouter({
  ...
  allUsers: withPermission("member", "read").query(async ({ ctx }) => {
    if (!isSuperAdmin(ctx.user)) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Only platform super administrators can list all users",
      });
    }
    ...
    const subscriptions =
      userIds.length === 0
        ...
        : await db.query.subscription.findMany({
            where: inArray(subscriptionTable.userId, userIds),
          });
    ...
    return memberships.map((m) => ({
      ...
      user: {
        ...m.user,
        subscription: subscriptionByUserId.get(m.userId) ?? null,
      },
    }));
  }),
  ...
  get: protectedProcedure.query(async ({ ctx }): Promise<UserGetOutput> => {
    const memberResult = await db.query.member.findFirst({
      where: and(
    ...
    if (isSuperAdminRes && memberResult) {
      return { ...memberResult, isSuperAdmin: isSuperAdminRes };
    }
    return memberResult;
  }),
