...
/** Продакшен API Т-Банка; для тестового шлюза задайте TINKOFF_API_BASE_URL. */
const TINKOFF_API_BASE_URL_DEFAULT = "https://securepay.tinkoff.ru/v2";
...
export class TinkoffPayment {
  ...
  public async charge(params: {
    ...
    try {
      ...
      const chargeDisabledForTerminal =
        String(response.ErrorCode ?? "") === "10";
      const failureKind =
        status === "REJECTED" || chargeDisabledForTerminal
          ? ("REJECTED" as const)
          : ("UNKNOWN" as const);
  ...
  public async status(paymentId: string): Promise<{
    ...
    amount: number;
    orderId: string;
    /** Есть после успешной оплаты с Recurrent=Y (нужен для Charge). */
    rebillId?: string;
  }> {
    ...
    const rebillRaw = response.RebillId ?? response.RebillID;
    const rebillId =
      rebillRaw !== undefined &&
      rebillRaw !== null &&
      String(rebillRaw).trim() !== ""
        ? String(rebillRaw)
        : undefined;
  ...
  public verifyWebhook(body: Record<string, unknown>): boolean {
    const token = body.Token;
    if (typeof token !== "string" || token.length === 0) return false;

    // T-Bank: в подпись не входят Token и вложенные объекты Data / Receipt
